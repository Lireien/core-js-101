...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  interval = isStartIncluded ? `[${start}, ` : `(${start}, `;
  interval = isEndIncluded ? `${interval}${finish}]` : `${interval}${finish})`;
  return interval;
}
...
function isCreditCardNumber(ccn) {
  const cnnReversed = ccn.toString().split('').reverse();
  const tempArr = cnnReversed.map((number, index) => {
    if (index % 2 === 0) {
      return number;
    }
    const tempNum = number * 2;
    if (tempNum > 9) {
      return tempNum.toString().split('').reduce((acc, cur) => +acc + +cur);
    }
    return tempNum;
  });
  const sum = tempArr.reduce((acc, cur) => +acc + +cur);
  return sum % 10 === 0;
}
...
function getDigitalRoot(num) {
  let sum = num;
  while (sum > 9) {
    const arrNums = sum.toString().split('');
    sum = arrNums.reduce((acc, cur) => +acc + +cur);
    if (arrNums < 10) break;
  }
  return sum;
}
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1Rows; i += 1) result[i] = [];
  for (let j = 0; j < m2Cols; j += 1) {
    for (let i = 0; i < m1Rows; i += 1) {
      let t = 0;
      for (let k = 0; k < m2Rows; k += 1) t += m1[i][k] * m2[k][j];
      result[i][j] = t;
    }
  }
  return result;
